...
class commonObj extends dbHandler {
	...
	function GetModules() {
		$sqlModules = "SELECT distinct moduleName,menuOrder FROM ORS_tblMenu where moduleStat='A' order by menuOrder";
		return $this->getArrRes($this->execQry($sqlModules));
	}
	function GetSubModules($moduleName) {
		$sqlSubModules = "SELECT moduleId,label,page FROM ORS_tblMenu where moduleStat='A' AND moduleName ='$moduleName' order by moduleOrder ";
	...
	function getuserPages($userId) {
		$sqlPages ="Select pages from ORS_tblusers where userId='$userId'";
