...
class rcrDateRangeObj extends commonObj {
	...
	function mmsData($dateFrom,$dateTo,$orgId) {
        ...
        $truncTable = "truncate table tbl_rcr_invoice_{$orgId}";
        ...
        if($this->execQry($truncTable)){    
            for($x=0;$x<=$i;$x++){  
                ...
                $sql="
                insert into tbl_rcr_invoice_{$orgId}
	...
	function mmsDataRemvSpace($orgId) {
		$sql="
		update tbl_rcr_invoice_{$orgId} set Col001 = LTRIM(RTRIM(Col001))
		";
		$this->execQry($sql);
	}
    ...
    function remvLoadedInvFirst($orgId) {
        ...
        $sql="
        delete from tbl_rcr_invoice_{$orgId} 
        where cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)+'='+cast(Col006 as nvarchar)
        in (select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar)+'='+cast(INVOICE_AMOUNT as nvarchar) from tbl_ap_invoices_all_rcr_amount_{$orgId})
        ";
        $this->execQry($sql);
    }
	...
	/*function oracleData() {
		...
		if($this->execQry($truncTable)){
			...
			where ORAPROD.INVOICE_NUM in (select distinct(Col001) from tbl_rcr_invoice_{$orgId})
			";
			$this->execQry($turnOnAnsiNulls);
			$this->execQry($turnOnAnsiWarn);
			$this->execQry($sql);
	...
	function viewRcrInv($orgId) {
		
		$sql="
		select Col001,Col004,Col006 as Col006 from tbl_rcr_invoice_{$orgId} group by Col001,Col004,Col006
		";
	...
	function oracleData($orgId,$clearOra) {
		...
		$arrRcrInv = $this->viewRcrInv($orgId);
        ...
        if($clearOra == 'false'){
            foreach($arrRcrInv as $valRcrInv){
                $sql="
                insert into tbl_ap_invoices_all_rcr_amount_{$orgId} (INVOICE_NUM,SEGMENT1,INVOICE_AMOUNT)
            ...
            $truncTable = "truncate table tbl_ap_invoices_all_rcr_amount_{$orgId}";
	...
	function updFileNameDuplicate($orgId) {
        ...
        $sqlPj="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PJ{$curDate}_duplicate.301' where Col011 = 'PO'
        and Col005 in 
        (
        ...
        $sqlPg="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PG{$curDate}_duplicate.301' where Col011 = 'PO' 
        and Col005 in 
        (
        ...
        $sqlPc="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PC{$curDate}_duplicate.301' where Col011 = 'PO'
        and Col005 in 
        (
        ...
        $sqlDi="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'DI{$curDate}_duplicate.301' where Col011 = 'PO'
        and Col005 in 
        (
        ...
        $sqlFl="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'FL{$curDate}_duplicate.301' where Col011 = 'PO'
        and Col005 in 
        (
    ...
    function updFileNameCancelled($orgId) {
        ...
        $sqlPj="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PJ{$curDate}_cancelled.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 700
            and STRNAM NOT LIKE 'X%'
        ...
        and cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar) in (
            select cast(a.Col001 as nvarchar)+'-'+cast(a.Col004 as nvarchar) from tbl_rcr_invoice_{$orgId} a
            left join tbl_ap_invoices_all_rcr_amount_{$orgId} b
            on 
        ...
        $sqlPg="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PG{$curDate}_cancelled.301' where Col011 = 'PO' 
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr 
            LEFT JOIN
        ...
        $sqlPc="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PC{$curDate}_cancelled.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 302
            and STRNAM NOT LIKE 'X%'
        ...
        $sqlDi="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'DI{$curDate}_cancelled.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 810
            and STRNUM < 900
        ...
        $sqlFl="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'FL{$curDate}_cancelled.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 811
            and STRNUM < 900
    ...
    function updFileNameLoad($orgId) {
        ...
        $sqlPj="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PJ{$curDate}_{$curTime}.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 700
            and STRNAM NOT LIKE 'X%'
        ...
        $sqlPg="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PG{$curDate}_{$curTime}.301' where Col011 = 'PO' 
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr 
            LEFT JOIN
        ...
        $sqlPc="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'PC{$curDate}_{$curTime}.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 302
            and STRNAM NOT LIKE 'X%'
        ...
        $sqlDi="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'DI{$curDate}_{$curTime}.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 810
            and STRNUM < 900
        ...
        $sqlFl="
        update tbl_rcr_invoice_{$orgId} set Col040 = 'FL{$curDate}_{$curTime}.301' where Col011 = 'PO'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 811
            and STRNUM < 900
	...
	function viewFileName($orgId) {
		...
		FROM tbl_rcr_invoice_{$orgId}
		";
		return $this->getArrRes($this->execQry($sql));
	}
	
	function viewRcr($orgId,$fileName) {
