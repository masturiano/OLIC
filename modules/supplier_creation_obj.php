...
class supplierObj extends commonObj {    
    ...
    function viewDataPj($supplier_number,$store_short) {
        ...
        $exclude = "AND TBLSTR.STSHRT not in ('SHAWDC','HO','ANGPC')";
        $forSetup = "AND TBLSTR.STSHRT not in ('CUB')"; 
        ...
        $sql_trunc_APSUPP = "
            IF OBJECT_ID('tbl_supplier_temp_APSUPP', 'U') IS NOT NULL
            BEGIN
            DROP TABLE tbl_supplier_temp_APSUPP
            END
        ";
        $sql_insert_APSUPP = "
            select *  into tbl_supplier_temp_APSUPP from openquery(pgjda, 'select * from mm760lib.APSUPP where ASNUM = ''{$supplier_number}''') APSUPP
        ";
        
        $sql_trunc_TXDFT = "  
            IF OBJECT_ID('tbl_supplier_temp_TXDFT', 'U') IS NOT NULL
            BEGIN
            drop table tbl_supplier_temp_TXDFT
            END
        ";
        $sql_insert_TXDFT = "
            select * into tbl_supplier_temp_TXDFT from openquery(pgjda, 'select * from mm760lib.TXDFT where TXNUM = ''{$supplier_number}''') TXDFT
        ";
        
        $sql_trunc_APADDR = "
            IF OBJECT_ID('tbl_supplier_temp_APADDR', 'U') IS NOT NULL
            BEGIN
            drop table tbl_supplier_temp_APADDR
            END
        ";
        $sql_insert_APADDR = "
            select * into tbl_supplier_temp_APADDR  from openquery(pgjda, 'select * from mm760lib.APADDR where AANUM = ''{$supplier_number}''') APADDR
        ";
        
        $sql_trunc_INVADIx1 = "
            IF OBJECT_ID('tbl_supplier_temp_INVADIx1', 'U') IS NOT NULL
            BEGIN
            drop table tbl_supplier_temp_INVADIx1
            END
        ";
        $sql_insert_INVADIx1 = "
            select * into tbl_supplier_temp_INVADIx1 from openquery(pgjda, 'select * from mm760lib.INVADIx1') INVADIx1
        ";    
        ...
        $sql="
            ...
            CASE WHEN ltrim(rtrim(ISNULL(APADDR.AAPHON,'NA'))) = '' THEN 'NA' ELSE left(ltrim(rtrim(ISNULL(APADDR.AAPHON,'NA'))),8) END as DTL_PHONE_NUMBER, --Line29
            '' as DTL_FAX_AREA_CODE, --Line30
            CASE WHEN ltrim(rtrim(ISNULL(APADDR.AAFAXN,'NA'))) = '' THEN 'NA' ELSE left(ltrim(rtrim(ISNULL(APADDR.AAFAXN,'NA'))),8) END as DTL_FAX_NUMBER, --Line31
            ...
            CASE WHEN ltrim(rtrim(ISNULL(APADDR.AACONT,'NA'))) = '' THEN 'NA' ELSE ltrim(rtrim(ISNULL(APADDR.AACONT,'NA'))) END as DTL2_CONT_FIRST_NAME, --Line52
            'NA' as DTL2_CONT_MIDDLE_NAME, --Line53
            'NA' as DTL2_CONT_LAST_NAME, --Line54
            'NA' as DTL2_CONT_PREFIX, --Line55
            ...
            'NA' as POSTAL_CODE, --Line61
            'NA' as CITY, --Line62
            'NA' as COUNTY, --Line63
            'NA' as STATE,     --Line64
            CASE WHEN ltrim(rtrim(ISNULL(APSUPP.ASGSTN,'NA'))) = '' THEN 'NA' ELSE ltrim(rtrim(ISNULL(APSUPP.ASGSTN,'NA'))) END as ASGSTN,  --Line65
            '' as LINE66,  --Line66
            '0' as LINE67, --Line67
            '' as LINE68, --Line68
            '0' as LINE69, --Line69
            cast('{$suffix}{$curDate}_0'+cast(APSUPP.ASNUM as varchar)+'.101' as nvarchar) as FILENAME
            FROM  tbl_supplier_temp_APSUPP APSUPP LEFT OUTER JOIN
                        tbl_supplier_temp_TXDFT  TXDFT ON apsupp.ASNUM = TXDFT.TXNUM LEFT OUTER JOIN
                        tbl_supplier_temp_APADDR APADDR ON apsupp.ASNUM = APADDR.AANUM INNER JOIN
                        tbl_supplier_temp_INVADIx1 INVADIx1 CROSS JOIN
                        sql_mmpgtlib..TBLSTR TBLSTR ON INVADIx1.STRNUM = TBLSTR.STRNUM
            WHERE   
        ...
        if($this->execQry($sql_trunc_APSUPP)){
            if($this->execQry($sql_insert_APSUPP)){
                if($this->execQry($sql_trunc_TXDFT)){
                    if($this->execQry($sql_insert_TXDFT)){
                        if($this->execQry($sql_trunc_APADDR)){
                            if($this->execQry($sql_insert_APADDR)){
    ...
    function viewDataDi($supplier_number,$store_short) {
        ...
        ";
        $sql_insert_APSUPP = "
            select *  into tbl_supplier_temp_APSUPP from openquery(pgjda, 'select * from mmneslib.APSUPP where ASNUM = ''{$supplier_number}''') APSUPP
        ";
        ...
        ";
        $sql_insert_TXDFT = "
            select * into tbl_supplier_temp_TXDFT from openquery(pgjda, 'select * from mmneslib.TXDFT where TXNUM = ''{$supplier_number}''') TXDFT
        ";
        ...
        ";
        $sql_insert_APADDR = "
            select * into tbl_supplier_temp_APADDR  from openquery(pgjda, 'select * from mmneslib.APADDR where AANUM = ''{$supplier_number}''') APADDR
        ";
        ...
        ";
        $sql_insert_INVADIx1 = "
            select * into tbl_supplier_temp_INVADIx1 from openquery(pgjda, 'select * from mmneslib.INVADIx1') INVADIx1
        ";   
    ...
    function findSupplier($terms){
        ...
        $sql = "
            select 
                DISTINCT TOP 10  asnum,asname 
            from 
            (
                    select 
                        asnum,asname 
                    from 
                        openquery(pgjda,'select asnum,asname from mm760lib.apsupp
                            where 
                                asnum  LIKE ''$terms%'' or asname  LIKE ''%$terms%''
                        ')
                    union all
                    select 
                        asnum,asname 
                    from 
                        openquery(pgjda,'select asnum,asname from mmneslib.apsupp
                            where 
                                asnum  LIKE ''$terms%'' or asname  LIKE ''%$terms%''
                        ')
            ) as apsupp
