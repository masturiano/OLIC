...
class a01FileObj extends commonObj {
	...
	function removeDoubleCreate(){
		...
		$nqryx = "DELETE
					...
					dbo.ar_tmp_mms
					WHERE WINVNO IN(
						SELECT WINVNO FROM #UnloadedAR
						GROUP BY WINVNO
						HAVING COUNT(WINVNO) > 1
					)";
		$cqryx = $this->execQry($nqryx);
		$stat = FALSE;
		if($cqryx){
			$stat = TRUE;
		}
	...
	function getLoadedInvoice(){
		//remove loaded invoice, regardles of customer and Site
		$nqry = "SELECT
					A.WINVNO,
					A.WFNAME
					INTO #UnloadedAR
		...
		$nqryx = "SELECT WINVNO,LEFT(WFNAME,2) as extname FROM #UnloadedAR
				GROUP BY WINVNO,LEFT(WFNAME,2)";
		
		$cqry = $this->execQry($nqryx);
		$orgId = array(
				'PG' => '85',
				'PJ' => '87',
				'PC' => '133',
		...
		$this->execQry("TRUNCATE TABLE ar_tmp_mms_duplicates");
		while($x = mssql_fetch_array($cqry))
		{
			$extn = $x['extname'];
			$inv = $x['WINVNO'];
			
			$ins = "INSERT INTO ar_tmp_mms_duplicates SELECT TRX_NUMBER,'".$extn."' as extname FROM OPENQUERY(ORAPROD,'
			...
			SELECT DISTINCT 
					...
					WHERE 
					RA_CUSTOMER_TRX_ALL.TRX_NUMBER = ''$inv''
			')";
			$this->execQry("SET ANSI_NULLS ON");
			$this->execQry("SET ANSI_WARNINGS ON");
			$q2 = $this->execQry($ins);
	...
	function removeLoadedInvoice(){
		$del = "DELETE FROM ar_tmp_mms
			WHERE WINVNO IN (
			SELECT WINVNO FROM ar_tmp_mms_duplicates
			)";
		$qry = $this->execQry($del);
		$stat = FALSE;
		if($qry){
			$stat = TRUE;
		}
