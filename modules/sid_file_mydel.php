...
switch($_POST['action']){
	...
	case "mmsDataRemvSpace":
		$sidFileObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$sidFileObj->oracleData();
	exit();
	
	case "remvLoadedInv":
		$sidFileObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$sidFileObj->updFileName();
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $sidFileObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $sidFileObj->viewOlicInv($fileName);
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    ...
                    <table border="0">
                    	<th colspan="2">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> 001 - SID (Textfiles) </font></h3>
                        </th>
                        <tr>
                            <td colspan="2">
                                <center>
                                    <br />
                                    <button class="btn btn-success" onClick="process5();" value="Analyze">  Process  </button>
                                </center>
                            </td>
                        </tr>
                    </table>
