...
include("rfp_file2_obj.php");

$rfpFileObj = new rfpFileObj();

switch($_POST['action']){

	case "rfpData":
	
		$checkReqId = $rfpFileObj->checkReqId($_POST['requestId']);
		
		if($checkReqId['request_id'] == '1'){
			echo "1";
		}else{
            
            $rfpFileObj->insertReqId($_POST['requestId']);
		
		    $directory="C:/wamp/www/OLIC/rfp_data/";
			$arch_directory="C:/wamp/www/OLIC/rfp_archive/";
			...
			if ($checkEmpty == "Empty"){
						...
						if($file_ext == "G01"){
							...
							if (($handle = fopen($csv_file, "r")) !== FALSE) {
								...
								while (($data = fgetcsv($handle, 10000000, "|")) !== FALSE) {
									...
									$rfpFileObj->rfpData($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col15,$col16,$col17,$col18,$col19,$col20,$col21,$col22,$col23,$col24,$col25,$col26,$col27,$col28,$col29,$col30,$col31,$col32,$col33,$col34,$col35,$col36,$col37,$col38,$col39,$col40);
								}	
								if($loaded = $rfpFileObj->displayLoadedRec()){
									echo "
				...
				$dir = 'C:/wamp/www/OLIC/rfp_data/';
				foreach(glob($dir.'*.G01*') as $v){
					unlink($v);
				}
				foreach(glob($dir.'*.G01*') as $v){
					unlink($v);
	...
	case "mmsDataRemvSpace":
		if($rfpFileObj->mmsDataRemvSpace($_POST['requestId'])){
            if($rfpFileObj->sumMmsData($_POST['requestId'])){
                $rfpFileObj->countMmsInvData($_POST['requestId']);
            }
        }
	exit();
	break;	
	...
	case "oracleData":
		if($rfpFileObj->oracleData($_POST['requestId'])){
            if($rfpFileObj->sumOracleData($_POST['requestId'])){
                $rfpFileObj->countOracleInvData($_POST['requestId']);
            }
        }
	exit();
	
	case "remvLoadedInv":
		$rfpFileObj->remvLoadedInv($_POST['requestId']);
	exit();
	break;	
	
	case "updFileName":
		$rfpFileObj->updFileName($_POST['requestId']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rfpFileObj->viewFileName($_POST['requestId']);
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $rfpFileObj->viewRfp($fileName);
			...
			function process()
			{
				...
				if(reqId == ''){
					...
					({
						url: 'rfp_file2.php',
						type: 'POST',
						data: 'action=rfpData&requestId='+reqId,
						beforeSend: function()
						{
						...
						success: function(data1){
							if(data1 == 1){
								...
								$().toastmessage('showToast', {
									text: 'Copy G01 (RFP) data success!',
									sticky: false,
									position: 'middle-center',
									type: 'success',
									closeText: '',
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    ...
                    <table border="0">
                    	<th colspan="3">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> G01 - RFP (Textfiles) w/ Request Id </font></h3>
                        </th>
						<tr>
