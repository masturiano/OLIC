...
switch($_POST['action']){
	...
	case "rcrData":
        ...
        $checkReqId = $rcrFileObj->checkReqId($_POST['requestId']);
		
        if($checkReqId['request_id'] == '1'){
            echo "1";
        }else{
            
            $rcrFileObj->insertReqId($_POST['requestId']);
            
            $directory="C:/wamp/www/OLIC/rcr_data/";
			$arch_directory="C:/wamp/www/OLIC/rcr_archive/";
			...
			if ($checkEmpty == "Empty"){
						...
						if($file_ext == "301"){
							...
							if (($handle = fopen($csv_file, "r")) !== FALSE) {
								...
								while (($data = fgetcsv($handle, 10000000, "|")) !== FALSE) {
									...
									$rcrFileObj->rcrData($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col15,$col16,$col17,$col18,$col19,$col20,$col21,$col22,$col23,$col24,$col25,$col26,$col27,$col28,$col29,$col30,$col31,$col32,$col33,$col34,$col35,$col36,$col37,$col38,$col39,$col40);
								}	
				...
				$dir = 'C:/wamp/www/OLIC/rcr_data/';
				foreach(glob($dir.'*.301*') as $v){
					unlink($v);
				}
				foreach(glob($dir.'*.301*') as $v){
					unlink($v);
	...
	case "mmsDataRemvSpace":
        if($rcrFileObj->mmsDataRemvSpace($_POST['requestId'])){
            if($rcrFileObj->sumMmsData($_POST['requestId'])){
                $rcrFileObj->countMmsInvData($_POST['requestId']);
            }
        }
	exit();
	break;	
	...
	case "oracleData":
		if($rcrFileObj->oracleData($_POST['requestId'])){
            if($rcrFileObj->sumOracleData($_POST['requestId'])){
                $rcrFileObj->countOracleInvData($_POST['requestId']);
            }
        }
	exit();
	
	case "remvLoadedInv":
		$rcrFileObj->remvLoadedInv($_POST['requestId']);
	exit();
	break;	
	
	case "updFileName":
		$rcrFileObj->updFileName($_POST['requestId']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rcrFileObj->viewFileName($_POST['requestId']);
            ...
            function process()
            {
                ...
                if(reqId == ''){
                    ...
                    ({
                        url: 'rcr_file2.php',
                        type: 'POST',
                        data: 'action=rcrData&requestId='+reqId,
                        beforeSend: function()
                        {
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    ...
                    <table border="0">
                    	<th colspan="3">
							<h3 align="center"><font style="font-family:Lucida Handwriting"> 301 - RCR (Textfiles) w/ Request Id </font></h3>
                        </th>
                        <tr>
