...
switch($_POST['action']){
	...
	case "mmsDataRemvSpace":
		$olicDateRangeObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$olicDateRangeObj->oracleData($_POST['orgId']);
	exit();
	break;	
	
	case "remvLoadedInv":
		$olicDateRangeObj->remvLoadedInv();
	exit();
	break;	
	...
	case "createTxtFile":
		...
		$arrFileName = $olicDateRangeObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $olicDateRangeObj->viewOlic($fileName);
