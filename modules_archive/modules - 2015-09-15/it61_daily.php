...
include("it61_daily_obj.php");

$it61DailyObj = new it61DailyObj();

switch($_POST['action']){
	
	case "mmsData":
		$it61DailyObj->mmsData($_POST['getTxtDate']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$it61DailyObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$it61DailyObj->oracleData();
	exit();
	break;	
	
	case "remvLoadedInv":
		$it61DailyObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$it61DailyObj->updFileName();
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $it61DailyObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $it61DailyObj->viewIt61($fileName);
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    <table border="0">
                    	<th colspan="2">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> 901 - IT61 (Daily) </font></h3>
                        </th>
                        <tr>
                            <td>
                                MMS Data:
