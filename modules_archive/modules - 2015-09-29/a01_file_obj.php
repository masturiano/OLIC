...
class a01FileObj extends commonObj {
	...
	function compareARlocaltoOracle(){
		...
		$nqry = "SELECT
					...
					FROM
					dbo.ar_tmp_mms AS A
					LEFT JOIN dbo.ra_customer_trx_all AS B ON A.WINVNO = B.TRX_NUMBER AND A.WCUSNO = B.ACCOUNT_NUMBER AND A.WBRHCD = B.LOCATION AND A.WDTAMT = B.AMOUNT_DUE_ORIGINAL
		...
		while($x = mssql_fetch_array($cqry))
		{
			...
			$fromFileName="../../IIC/EXPORTEDFILE/".$filen;
