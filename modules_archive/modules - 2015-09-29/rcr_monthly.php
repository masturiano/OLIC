...
switch($_POST['action']){
	...
	case "mmsData":
		$rcrMonthlyObj->mmsData($_POST['getTxtMonYr'],$_POST['orgId']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$rcrMonthlyObj->mmsDataRemvSpace($_POST['orgId']);
	exit();
	break;	
    
    case "remvLoadedInvFirst":
        $rcrMonthlyObj->remvLoadedInvFirst($_POST['orgId']);
    exit();
    break; 
	
	case "oracleData":
		$rcrMonthlyObj->oracleData($_POST['orgId'],$_POST['clearOra']);
	exit();
	
	case "remvLoadedInv":
		$rcrMonthlyObj->remvLoadedInv($_POST['orgId']);
	exit();
	break;	
	
	case "updFileName":
		$rcrMonthlyObj->updFileName($_POST['orgId']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rcrMonthlyObj->viewFileName($_POST['orgId']);
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $rcrMonthlyObj->viewRcr($fileName,$_POST['orgId']);
            ...
            <div id="activity_pane" align="center">
                ...
                <table border="0">
                    ...
                    <tr>
                        ...
                        <td><? $rcrMonthlyObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox"'); ?></td>
                    </tr>
                    <tr>
