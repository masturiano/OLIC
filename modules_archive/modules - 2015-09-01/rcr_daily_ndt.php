...
include("rcr_daily_ndt_obj.php");

$rcrDailyObj = new rcrDailyObj();

switch($_POST['action']){
	
	case "mmsData":
		$rcrDailyObj->mmsData($_POST['getTxtDate'],$_POST['orgId']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$rcrDailyObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$rcrDailyObj->oracleData($_POST['orgId']);
	exit();
	break;	
	
	case "remvLoadedInv":
		$rcrDailyObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$rcrDailyObj->updFileName($_POST['orgId']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rcrDailyObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $rcrDailyObj->viewRcr($fileName);
...
<html>
	<head>
        ...
        <style type="text/css">
            ...
            table#ndtSuppliers tr.even td {
            background-color: white;
            }

            table#ndtSuppliers tr.odd td {
                background-color: skyblue;
            }
            
            table#ndtSuppliers tr.even:hover td {
                background-color: lightgray;
            }

            table#ndtSuppliers tr.odd:hover td {
                background-color: lightgray;
            }
            ...
            <div id="activity_pane" align="center">
                ...
                <table border="0">
                    ...
                    <tr>
                        ...
                        <td><? $rcrDailyObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox"'); ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="3">
                            <center>
                                <br />
                                <button class="btn btn-success" onClick="process();" id="process" class="btn" value="Analyze"> Process </button>
                            </center>
                        </td>
                    </tr>
                </table>
                <br /><br />
                <font style="font-family:Lucida Handwriting; font-size: 10px;"> 
                    <b>LIST OF NDT SUPPLIERS</b>
                </font>  
                
                <div id="ndtSuppliersBorder">
                    <table id="ndtSuppliers" class="ndtSuppliers" align="center" width="100%">
                         <thead>
                            <tr>
                        ...
                        <tbody> 
                            <?
                            $listOfSup = $rcrDailyObj->ndtSuppliers();
                            foreach($listOfSup as $valSup){
                            ?>      
                            <tr>
                                <td align="left"><? echo $valSup['SUPPLIER_NO']; ?></td>
                                <td align="left"><? echo $valSup['ASNAME']; ?></td>
                            </tr>
                            <?
                            }
                            ?>
