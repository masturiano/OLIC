...
include("rcr_monthly_obj.php");

$rcrMonthlyObj = new rcrMonthlyObj();

switch($_POST['action']){
	
	case "mmsData":
		$rcrMonthlyObj->mmsData($_POST['getTxtMonYr']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$rcrMonthlyObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$rcrMonthlyObj->oracleData($_POST['getTxtMonYr']);
	exit();
	
	case "remvLoadedInv":
		$rcrMonthlyObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$rcrMonthlyObj->updFileName();
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rcrMonthlyObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $rcrMonthlyObj->viewRcr($fileName);
...
<html>
	<head>
		...
		<script src="../includes/mini-upload-form/assets/js/jquery.knob.js"></script>
		<script src="../includes/mini-upload-form/assets/js/jquery.ui.widget.js"></script>
		<script src="../includes/mini-upload-form/assets/js/jquery.iframe-transport.js"></script>
		<script src="../includes/mini-upload-form/assets/js/jquery.fileupload.js"></script>
		<script src="../includes/mini-upload-form/assets/js/script.js"></script>
        <link href="../includes/mini-upload-form/assets/css/style.css" rel="stylesheet" />
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    ...
                    <table border="0">
                    	<th colspan="2">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> 601 - Concess (Textfiles) </font></h3>
                        </th>
                        <tr>
                            <td>
								<div class="dvContainer2" style="float:left; height:100%; width:100%;" align="center">
								
									<form id="upload" method="post" action="con_upload.php" enctype="multipart/form-data">
										<div id="drop">
											<!-- Drop Here -->
											<a>Browse</a>
