...
include("supplier_obj.php");

$supplierObj = new supplierObj();

$arrSuppList = $supplierObj->viewSupplier();

switch($_POST['action']){
	
	case "mmsData":
		$supplierObj->mmsData($_POST['orgId']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$supplierObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$supplierObj->oracleData($_POST['orgId']);
	exit();
	
	case "remvLoadedInv":
		$supplierObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$supplierObj->updFileName();
	exit();
	break;	
	...
	case "createTextFile":
        ...
        if($_POST['orgId'] == '87'){
            $arrData = $supplierObj->viewDataPj($_POST['supplierNo']);
        }else if($_POST['orgId'] == '85'){
            $arrData = $supplierObj->viewDataPg($_POST['supplierNo']);
        }else if($_POST['orgId'] == '133'){
            $arrData = $supplierObj->viewDataPc($_POST['supplierNo']);
        }else if($_POST['orgId'] == '153'){
            $arrData = $supplierObj->viewDataDi($_POST['supplierNo']);  
        }else if($_POST['orgId'] == '113'){
            $arrData = $supplierObj->viewDataFl($_POST['supplierNo']);  
        }else{
            $arrData = "Error!";
        }
        ...
        <script type="text/javascript">
			...
			function createText(supnum)
            {
                ...
                ({
                    url: 'supplier.php',
                    type: 'POST',
                    data: 'action=createTextFile&orgId='+orgId+'&supplierNo='+supnum,
                    beforeSend: function()
                    {
                        $("#create-"+supnum).html('CREATING TEXTFILE').css('color','white').css("text-decoration", "blink");
                        $("#create-"+supnum).attr('disabled','disabled');
                    },
                    success: function(data1){
                        $().toastmessage('showToast', {
                            text: 'Textfile created for supplier no. '+supnum,
                            sticky: true,
                            position: 'middle-center',
                            type: 'success',
                            closeText: '',
                        ...
                        $("#create-"+supnum).html('CREATE TEXTFILE').css("text-decoration", "none");
                        $("#create-"+supnum).removeAttr('disabled');
                    }
                });    
            }
                    ...
                    <div id="ndtSuppliersBorder">
                        <table border="0">
                            <tr>
                                <td colspan="3" align="center">  
                                    <font style="font-family:Lucida Handwriting; font-size: 15px;"> 
                                        <b>MMS Supplier not in Oracle</b>
                                    </font>   
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td><? $supplierObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox"'); ?></td>
                            </tr>
                            <tr>
                                <td colspan="3">
                                    <center>
                        ...
                        <table id="supplierList" align="center">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td align="left"><?=$val['asnum']?></td>
                                    <td align="left"><?=$val['asname']?></td>
                                    <td align="left">
                                        <button class="create" id="<?="create-".$val['asnum']; ?>" onClick="createText(<? echo $val['asnum']; ?>);" value="Analyze"> CREATE TEXTFILE </button>
                                        <div id="<?="divCusnum-".$val['asnum']; ?>" align="center">
                                        </div>
                                    </td>
                                </tr>
                                <?php }?>
