...
class conFileObj extends commonObj {
	...
	function oracleData() {
		...
		$truncTable = "truncate table tbl_ap_invoices_all_con";
		
		if($this->execQry($truncTable)){
		
			foreach($arrConInv as $valConInv){
				$sql="
				insert into tbl_ap_invoices_all_con (INVOICE_NUM,SEGMENT1)
	...
	function remvLoadedInv() {
		...
		in (select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar) from tbl_ap_invoices_all_con)
		";
		$this->execQry($sql);
	}
