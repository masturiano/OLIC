...
class stsFileObj extends commonObj {
	...
	function sumMmsData($reqId) {
		...
		(select sum(cast(Col013 as float)) as uniqAmt,cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar) as uniqInv from tbl_sts_invoice group by cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)) as tbl_sts_invoice
		";
	...
	function countMmsInvData($reqId) {
		...
		(select cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar) as uniqInv from tbl_sts_invoice group by cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)) as tbl_sts_invoice
		";
	...
	function viewStsInv($reqId) {
		
		$sql="
		select Col001,Col004,sum(cast(Col013 as float)) as Col013 from tbl_sts_invoice group by Col001,Col004
		";
		...
		if($this->execQry($truncTable)){
			...
			foreach($arrStsInv as $valInv){
				...
				select ORAPROD.INVOICE_NUM,ORAPROD.SEGMENT1,ORAPROD.INVOICE_AMOUNT from openquery(ORAPROD,
					...
					where ap_invoices_all.INVOICE_NUM = ''{$valInv[Col001]}''
					and ap_suppliers.SEGMENT1 = ''{$valInv[Col004]}''
					and ap_invoices_all.INVOICE_AMOUNT = ''{$valInv[Col013]}''
					'
					) ORAPROD
				";
	...
	function sumOracleData($reqId) {
		$sql="
		select sum(INVOICE_AMOUNT) AS INVOICE_AMOUNT from  tbl_ap_invoices_all_sts_amount
		";
	...
	function countOracleInvData($reqId) {
		...
		(select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar) as uniqInv from tbl_ap_invoices_all_sts_amount group by cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar)) as tbl_ap_invoices_all_sts_amount
		";
