...
include("rcr_file_obj.php");

$rcrFileObj = new rcrFileObj();
...
switch($_POST['action']){
	...
	case "rcrData":
         ...
         if($_POST['orgId'] == '87'){
            $directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_pj_data/";  
            $arch_directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_pj_archive/";  
            $orgName = "PJ";     
        }else if($_POST['orgId'] == '85'){
            $directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_pg_data/";  
            $arch_directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_pg_archive/";
            $orgName = "PG";  
        }else if($_POST['orgId'] == '133'){
            $directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_sbc_data/";  
            $arch_directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_sbc_archive/"; 
            $orgName = "PC"; 
        }else if($_POST['orgId'] == '153'){
            $directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_di_data/";  
            $arch_directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_di_archive/";  
            $orgName = "DI";  
        }else if($_POST['orgId'] == '113'){
            $directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_fl_data/";  
            $arch_directory="C:/wamp/www/OLIC/imported_file/rcr/rcr_fl_archive/"; 
            $orgName = "FL";   
        }else{
            $directory="";  
            $arch_directory="";
			...
			if ($checkEmpty == "Empty"){
                            ...
                            if($orgName == $fileType){
							    ...
							    if (($handle = fopen($csv_file, "r")) !== FALSE) {
								    ...
								    while (($data = fgetcsv($handle, 10000000, "|")) !== FALSE) {
									    ...
									    $rcrFileObj->olicData($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col15,$col16,$col17,$col18,$col19,$col20,$col21,$col22,$col23,$col24,$col25,$col26,$col27,$col28,$col29,$col30,$col31,$col32,$col33,$col34,$col35,$col36,$col37,$col38,$col39,$col40);
								    }	
								    if($loaded = $rcrFileObj->displayLoadedRec()){
									    echo "
				...
				foreach(glob($directory.'*.301*') as $v){
					unlink($v);
				}
				foreach(glob($directory.'*.301*') as $v){
					unlink($v);
	...
	case "mmsDataRemvSpace":
		$rcrFileObj->mmsDataRemvSpace();
	exit();
	break;	
    
    case "remvLoadedInvFirst":
        $rcrFileObj->remvLoadedInvFirst();
    exit();
    break; 
	
	case "oracleData":
		$rcrFileObj->oracleData($_POST['orgId'],$_POST['clearOra']);
	exit();
	
	case "remvLoadedInv":
		$rcrFileObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileNameDuplicate":
        $rcrFileObj->updFileNameDuplicate($_POST['orgId']);
	exit();
	break;	
    
    case "updFileNameCancelled":
        $rcrFileObj->updFileNameCancelled($_POST['orgId']);
    exit();
    break;    
    
    case "updFileNameLoad":
        $rcrFileObj->updFileNameLoad($_POST['orgId']);
    exit();
    break;    
	
	case "createTxtFile":
	
		$arrFileName = $rcrFileObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $rcrFileObj->viewRcr($fileName);
        ...
        <script type="text/javascript">
			function process()
			{
				...
				({
					url: 'rcr_file.php',
					type: 'POST',
					data: 'action=rcrData&orgId='+orgId+'&clearOra='+clearOraInv,
					beforeSend: function()
					{
			...
			function process2(orgId,clearOraInv)
			{
				$.ajax
				({
					url: 'rcr_file.php',
					type: 'POST',
					data: 'action=mmsDataRemvSpace&orgId='+orgId+'&clearOra='+clearOraInv,
					success: function(data1){
						$().toastmessage('showToast', {
						...
						if(clearOraInv == false){
                            process3(orgId,clearOraInv); 
                        }else{
                            process4(orgId,clearOraInv);     
                        }
					}
				});	
			}
            
            function process3(orgId,clearOraInv)
            {
                $.ajax
                ({
                    url: 'rcr_file.php',
                    type: 'POST',
                    data: 'action=remvLoadedInvFirst&orgId='+orgId+'&clearOra='+clearOraInv,
                    success: function(data1){
                        $().toastmessage('showToast', {
			...
			function process4(orgId,clearOraInv)
			{
				$.ajax
				({
					url: 'rcr_file.php',
					type: 'POST',
					data: 'action=oracleData&orgId='+orgId+'&clearOra='+clearOraInv,
					success: function(data1){
						$().toastmessage('showToast', {
						...
						process5(orgId,clearOraInv);
					}
				});	
			}
			
			function process5(orgId,clearOraInv)
			{
				$.ajax
				({
					url: 'rcr_file.php',
					type: 'POST',
					data: 'action=remvLoadedInv&orgId='+orgId+'&clearOra='+clearOraInv,
					success: function(data1){
						$().toastmessage('showToast', {
							text: 'Removing loaded invoices success!',
							sticky: false,
						...
						process6(orgId,clearOraInv);
					}
				});	
			}
			
			function process6(orgId,clearOraInv)
			{
				$.ajax
				({
					url: 'rcr_file.php',
					type: 'POST',
					data: 'action=updFileNameDuplicate&orgId='+orgId+'&clearOra='+clearOraInv,
					success: function(data1){
						$().toastmessage('showToast', {
						...
						process7(orgId,clearOraInv);
					}
				});	
			}
            
            function process7(orgId,clearOraInv)
            {
                $.ajax
                ({
                    url: 'rcr_file.php',
                    type: 'POST',
                    data: 'action=updFileNameCancelled&orgId='+orgId+'&clearOra='+clearOraInv,
                    success: function(data1){
                        $().toastmessage('showToast', {
                        ...
                        process8(orgId,clearOraInv);
                    }
                });    
            }
            
            function process8(orgId,clearOraInv)
            {
                $.ajax
                ({
                    url: 'rcr_file.php',
                    type: 'POST',
                    data: 'action=updFileNameLoad&orgId='+orgId+'&clearOra='+clearOraInv,
                    success: function(data1){
                        $().toastmessage('showToast', {
                        ...
                        process9(orgId,clearOraInv);
                    }
                });    
            }
			
			function process9(orgId,clearOraInv)
			{
				$.ajax
				({
					url: 'rcr_file.php',
					type: 'POST',
					data: 'action=createTxtFile&orgId='+orgId+'&clearOra='+clearOraInv,
					success: function(data1){
						$().toastmessage('showToast', {
            ...
            <div id="activity_pane" align="center">
                ...
                <table border="0">
                    ...
                    <tr>
                        ...
                        <td><? $rcrFileObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox"'); ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="3">
                            <center>
                                <br />
                                <button class="btn btn-success" onClick="process();" class="btn" value="Analyze"> Process </button>
                            </center>
                        </td>
                    </tr>
                </table>
