...
include("sid_daily_obj.php");

$sidDailyObj = new sidDailyObj();

switch($_POST['action']){
	
	case "mmsData":
		$sidDailyObj->mmsData($_POST['getTxtDate']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$sidDailyObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$sidDailyObj->oracleData();
	exit();
	break;	
	
	case "remvLoadedInv":
		$sidDailyObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$sidDailyObj->updFileName($_POST['getTxtDate']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $sidDailyObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $sidDailyObj->viewSid($fileName);
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    <table border="0">
                    	<th colspan="2">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> 001 - SID (Daily) </font></h3>
                        </th>
                        <tr>
                            <td>
                                MMS Data:
