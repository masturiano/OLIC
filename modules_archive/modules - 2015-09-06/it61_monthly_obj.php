...
class it61MonthlyObj extends commonObj {
	...
	function mmsData($monYear) {
		...
		$truncTable = "truncate table tbl_it61_invoice";
		
		if($this->execQry($truncTable)){
			$sql="
			insert into tbl_it61_invoice
	...
	function mmsDataRemvSpace() {
		$sql="
		update tbl_it61_invoice set Col001 = LTRIM(RTRIM(Col001))
		";
		$this->execQry($sql);
	}
	...
	function viewIt61Inv() {
		...
		$sql="
		select distinct(Col001) as Col001 from tbl_it61_invoice
		";
	...
	function oracleData($monYear) {
		...
		$truncTable = "truncate table tbl_ap_invoices_all_it61";
		
		if($this->execQry($truncTable)){
		
			foreach($arrIt61Inv as $valRcrInv){
				$sql="
				insert into tbl_ap_invoices_all_it61 (INVOICE_NUM,SEGMENT1)
				select ORAPROD.INVOICE_NUM,ORAPROD.SEGMENT1 from openquery(ORAPROD,
					...
					where ap_invoices_all.INVOICE_NUM = ''{$arrIt61Inv[Col001]}'''
					) ORAPROD
				";
				$this->execQry($turnOnAnsiNulls);
				$this->execQry($turnOnAnsiWarn);
	...
	function remvLoadedInv() {
		...
		$curDate =  date('mdy');
		$curTime =  date('hms');
		// or tbl_it61_invoice.Col040 like '%{$curDate}%'
		$sql="
		delete from tbl_it61_invoice 
		where cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)
		in (select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar) from tbl_ap_invoices_all_it61)
		";
		$this->execQry($sql);
	}
	...
	function updFileName() {
		...
		$updToRTV = "update tbl_it61_invoice set Col011 = 'RTV'";
		
		if($this->execQry($updToRTV)){
            
            /*
			$sql="
			update tbl_it61_invoice set Col040 = 'PJ{$curDate}_{$curTime}.901' where Col011 = 'RTV' and Col001 like 'IT61%' and Col025 = '700' and Col040 like 'PJ%'
			update tbl_it61_invoice set Col040 = 'PG{$curDate}_{$curTime}.901' where Col011 = 'RTV' and Col001 like 'IT61%' and Col025 = '101' and Col040 like 'PG%'
			update tbl_it61_invoice set Col040 = 'PC{$curDate}_{$curTime}.901' where Col011 = 'RTV' and Col001 like 'IT61%' and Col025 = '302' and Col040 like 'PC%'
			";
            */
	...
	function viewFileName() {
		...
		FROM tbl_it61_invoice
		";
		return $this->getArrRes($this->execQry($sql));
	}
	
	function viewIt61($fileName) {
