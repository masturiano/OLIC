...
include("sid_file2_obj.php");

$sidFileObj = new sidFileObj();

switch($_POST['action']){
	
	case "sidData":
		
		$checkReqId = $sidFileObj->checkReqId($_POST['requestId']);
		
		if($checkReqId['request_id'] == '1'){
			echo "1";
		}else{
		
			$sidFileObj->insertReqId($_POST['requestId']);
		
			$directory="C:/wamp/www/OLIC/sid_data/";
			$arch_directory="C:/wamp/www/OLIC/sid_archive/";
			...
			if ($checkEmpty == "Empty"){
						...
						if($file_ext == "001"){
							...
							if (($handle = fopen($csv_file, "r")) !== FALSE) {
								...
								while (($data = fgetcsv($handle, 10000000, "|")) !== FALSE) {
									...
									$sidFileObj->sidData($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col15,$col16,$col17,$col18,$col19,$col20,$col21,$col22,$col23,$col24,$col25,$col26,$col27,$col28,$col29,$col30,$col31,$col32,$col33,$col34,$col35,$col36,$col37,$col38,$col39,$col40);
								}	
								if($loaded = $sidFileObj->displayLoadedRec()){
									echo "
				...
				$dir = 'C:/wamp/www/OLIC/sid_data/';
				foreach(glob($dir.'*.001*') as $v){
					unlink($v);
				}
				foreach(glob($dir.'*.001*') as $v){
					unlink($v);
	...
	case "mmsDataRemvSpace":
		if($sidFileObj->mmsDataRemvSpace($_POST['requestId'])){
			if($sidFileObj->sumMmsData($_POST['requestId'])){
				$sidFileObj->countMmsInvData($_POST['requestId']);
			}
		}
	exit();
	break;	
	...
	case "oracleData":
		if($sidFileObj->oracleData($_POST['requestId'])){
			if($sidFileObj->sumOracleData($_POST['requestId'])){
				$sidFileObj->countOracleInvData($_POST['requestId']);
			}
		}
	exit();
	
	case "remvLoadedInv":
		$sidFileObj->remvLoadedInv($_POST['requestId']);
	exit();
	break;	
	
	case "updFileName":
		$sidFileObj->updFileName($_POST['requestId']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $sidFileObj->viewFileName($_POST['requestId']);
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $sidFileObj->viewSid($fileName);
	...
	<head>
        ...
        <script type="text/javascript">
			function process()
			{
				...
				if(reqId == ''){
				...
				else if(reqId.length < 8){
					$().toastmessage('showToast', {
						text: 'Request id must be 8 digits!',
						sticky: false,
						position: 'middle-center',
						type: 'error',
						closeText: '',
					...
					$.ajax
					({
						url: 'sid_file2_mydel.php',
						type: 'POST',
						data: 'action=sidData&requestId='+reqId,
						beforeSend: function()
						{
						...
						success: function(data1){
							if(data1 == 1){
								...
								$().toastmessage('showToast', {
									text: 'Copy 001 (SID) data success!',
									sticky: false,
									position: 'middle-center',
									type: 'success',
									closeText: '',
			...
			function process2(reqId)
			{
				$.ajax
				({
					url: 'sid_file2_mydel.php',
					type: 'POST',
					data: 'action=mmsDataRemvSpace&requestId='+reqId,
					success: function(data1){
						$().toastmessage('showToast', {
			...
			function process3(reqId)
			{
				$.ajax
				({
					url: 'sid_file2_mydel.php',
					type: 'POST',
					data: 'action=oracleData&requestId='+reqId,
					success: function(data1){
						$().toastmessage('showToast', {
			...
			function process4(reqId)
			{
				$.ajax
				({
					url: 'sid_file2_mydel.php',
					type: 'POST',
					data: 'action=remvLoadedInv&requestId='+reqId,
					success: function(data1){
						$().toastmessage('showToast', {
			...
			function process5(reqId)
			{
				$.ajax
				({
					url: 'sid_file2_mydel.php',
					type: 'POST',
					data: 'action=updFileName&requestId='+reqId,
					success: function(data1){
						$().toastmessage('showToast', {
			...
			function process6(reqId)
			{
				$.ajax
				({
					url: 'sid_file2_mydel.php',
					type: 'POST',
					data: 'action=createTxtFile&requestId='+reqId,
					success: function(data1){
						$().toastmessage('showToast', {
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
					...
					<table border="0">
						<th colspan="3">
							<h3 align="center"><font style="font-family:Lucida Handwriting"> 001 - SID (Textfiles) w/ Request Id </font></h3>
						</th>
						<tr>
							<td><font style="font-size:13px"> Request Id </font></td>
							<td><font style="font-size:13px"> : </font></td>
							<td><font style="font-size:13px"><input type="text" name="reqId" id="reqId" class="textBox" onKeyUp="numericFilter(this);" maxlength="8" style="height:20px;"/></font></td>
						</tr>
						<tr>
							<td colspan="3">
								<center>
									<br />
									<button class="btn btn-success" onClick="process6();" value="Analyze">  Process  </button>
								</center>
							</td>
						</tr>
					</table>
