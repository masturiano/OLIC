...
include("customer_zero_name_ora_obj.php");
...
switch($_POST['action']){
	...
	case "oracleData":
		$customerObj->oracleData();
	exit();
	
	case "createTextFile":
    
        # JUNIOR TEXTFILE
        if($arrData = $customerObj->viewDataPj($_POST['customerNo'])){
            ...
            # PPCI TEXTFILE
            if($arrData = $customerObj->viewDataPg($_POST['customerNo'])){
                foreach($arrData as $valData){
                ...
                # SUBIC TEXTFILE    
                if($arrData = $customerObj->viewDataPc($_POST['customerNo'])){
                    foreach($arrData as $valData){
                    ...
                    # DCI TEXTFILE 
                    if($arrData = $customerObj->viewDataDi($_POST['customerNo'])){
                        foreach($arrData as $valData){
                        ...
                        # FLS TEXTFILE 
                        if($arrData = $customerObj->viewDataFl($_POST['customerNo'])){
                            foreach($arrData as $valData){
...
<html>
	<head>
        ...
        <script type="text/javascript">
			...
			function process(orgId)
			{
				$.ajax
				({
					url: 'customer_zero_name_ora.php',
					type: 'POST',
            ...
            function createText(cusnum)
            {
                ...
                ({
                    url: 'customer_zero_name_ora.php',
                    type: 'POST',
                    data: 'action=createTextFile&customerNo='+cusnum,
                    beforeSend: function()
                    {
                            ...
                            <tr>
                                <td colspan="3" align="center">  
                                    <font style="font-family:Lucida Handwriting; font-size: 15px;"> 
                                        <b>MMS Customer with zero name in Oracle</b>
                                    </font>   
                                </td>
                            </tr>
                            <tr>
					    ...
					    <div id="dataTable">
						    ...
						    <tbody>
								    ...
								    <?php foreach ($arrCustList as $val) {?>
								        ...
								        <tr>
									        <td align="left"><?=$val['account_number']?></td>
                                            <td align="left"><?=$val['party_name']?></td>
									        <td align="left">
                                                <button class="create" id="<?="create-".$val['account_number']; ?>" onClick="createText(<? echo $val['account_number']; ?>);" value="Analyze"> CREATE TEXTFILE </button>
                                                <div id="<?="divCusnum-".$val['account_number']; ?>" align="center">
                                                </div>
                                            </td>
								        </tr>   
								    <?php }?>
