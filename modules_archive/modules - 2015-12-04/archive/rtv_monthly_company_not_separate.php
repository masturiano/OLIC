...
include("rtv_monthly_obj.php");

$rtvMonthlyObj = new rtvMonthlyObj();

switch($_POST['action']){
	
	case "mmsData":
		$rtvMonthlyObj->mmsData($_POST['getTxtMonYr']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$rtvMonthlyObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$rtvMonthlyObj->oracleData($_POST['getTxtMonYr']);
	exit();
	
	case "remvLoadedInv":
		$rtvMonthlyObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$rtvMonthlyObj->updFileName();
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rtvMonthlyObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $rtvMonthlyObj->viewRtv($fileName);
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    <table border="0">
                    	<th colspan="2">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> 901 - RTV (Monthly) </font></h3>
                        </th>
                        <tr>
                            <td>
                                MMS Data:
