...
class olicDailyObj extends commonObj {
	...
	function clearTblOlicInvoice(){
		$sql = "
		truncate table tbl_con_invoice
		";
	...
	function mmsDataRemvSpace() {
		$sql="
		update tbl_con_invoice set Col001 = LTRIM(RTRIM(Col001))
		";
		$this->execQry($sql);
	}
	...
	function viewOlicInv() {
		...
		$sql="
		select Col001,Col004,Col006 as Col006 from tbl_con_invoice group by Col001,Col004,Col006
		";
	...
	function oracleData($orgId) {
		...
		$truncTable = "truncate table tbl_ap_invoices_all_con_amount";
		
		if($this->execQry($truncTable)){
		
			foreach($arrOlicInv as $valOlicInv){
				$sql="
                insert into tbl_ap_invoices_all_con_amount (INVOICE_NUM,SEGMENT1,INVOICE_AMOUNT)
                select ORAPROD.INVOICE_NUM,ORAPROD.SEGMENT1,ORAPROD.INVOICE_AMOUNT from openquery(ORAPROD,
                    ...
                    and (ap_invoices_all.source = ''CON'' or ap_invoices_all.source = ''Manual Invoice Entry'') 
                    '
                    ) ORAPROD
                ";
	...
	function remvLoadedInv() {
		...
		$sql="        
        delete from tbl_con_invoice
        where cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)+'='+cast(Col006 as nvarchar)
        in (select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar)+'='+cast(INVOICE_AMOUNT as nvarchar) from tbl_ap_invoices_all_con_amount)
        ";
		$this->execQry($sql);
	}
	...
	function updFileNameDuplicate($orgId) {
        ...
        $sqlPj="
        update tbl_con_invoice set Col040 = 'PJ{$curDate}_duplicate.601' where Col011 = 'CON'
        and Col005 in 
        (
        ...
        $sqlPg="
        update tbl_con_invoice set Col040 = 'PG{$curDate}_duplicate.601' where Col011 = 'CON' 
        and Col005 in 
        (
        ...
        $sqlPc="
        update tbl_con_invoice set Col040 = 'PC{$curDate}_duplicate.601' where Col011 = 'CON'
        and Col005 in 
        (
        ...
        $sqlDi="
        update tbl_con_invoice set Col040 = 'DI{$curDate}_duplicate.601' where Col011 = 'CON'
        and Col005 in 
        (
        ...
        $sqlFl="
        update tbl_con_invoice set Col040 = 'FL{$curDate}_duplicate.601' where Col011 = 'CON'
        and Col005 in 
        (
    ...
    function updFileNameCancelled($orgId) {
        ...
        $sqlPj="
        update tbl_con_invoice set Col040 = 'PJ{$curDate}_cancelled.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 700
            and STRNAM NOT LIKE 'X%'
        ...
        and cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar) in (
            select cast(a.Col001 as nvarchar)+'-'+cast(a.Col004 as nvarchar) from tbl_con_invoice a
            left join tbl_ap_invoices_all_con_amount b
            on 
        ...
        $sqlPg="
        update tbl_con_invoice set Col040 = 'PG{$curDate}_cancelled.601' where Col011 = 'CON' 
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr 
            LEFT JOIN
        ...
        $sqlPc="
        update tbl_con_invoice set Col040 = 'PC{$curDate}_cancelled.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 302
            and STRNAM NOT LIKE 'X%'
        ...
        $sqlDi="
        update tbl_con_invoice set Col040 = 'DI{$curDate}_cancelled.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 810
            and STRNUM < 900
        ...
        $sqlFl="
        update tbl_con_invoice set Col040 = 'FL{$curDate}_cancelled.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 811
            and STRNUM < 900
    ...
    function updFileNameLoad($orgId) {
        ...
        $sqlPj="
        update tbl_con_invoice set Col040 = 'PJ{$curDate}_{$curTime}.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 700
            and STRNAM NOT LIKE 'X%'
        ...
        $sqlPg="
        update tbl_con_invoice set Col040 = 'PG{$curDate}_{$curTime}.601' where Col011 = 'CON' 
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr 
            LEFT JOIN
        ...
        $sqlPc="
        update tbl_con_invoice set Col040 = 'PC{$curDate}_{$curTime}.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmpgtlib.dbo.tblstr where STCOMP = 302
            and STRNAM NOT LIKE 'X%'
        ...
        $sqlDi="
        update tbl_con_invoice set Col040 = 'DI{$curDate}_{$curTime}.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 810
            and STRNUM < 900
        ...
        $sqlFl="
        update tbl_con_invoice set Col040 = 'FL{$curDate}_{$curTime}.601' where Col011 = 'CON'
        and Col005 in 
        (
            select STSHRT from sql_mmneslib.dbo.tblstr where STCOMP = 811
            and STRNUM < 900
