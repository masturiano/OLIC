...
switch($_POST['action']){
	...
	case "rtvData":
		...
		$checkReqId = $rtvFileObj->checkReqId($_POST['requestId']);
		
		if($checkReqId['request_id'] == '1'){
			echo "1";
		}else{
		
			$rtvFileObj->insertReqId($_POST['requestId']);
	...
	case "mmsDataRemvSpace":
		if($rtvFileObj->mmsDataRemvSpace($_POST['requestId'])){
			if($rtvFileObj->sumMmsData($_POST['requestId'])){
				$rtvFileObj->countMmsInvData($_POST['requestId']);
			}
		}
	exit();
	break;	
	...
	case "oracleData":
		if($rtvFileObj->oracleData($_POST['requestId'])){
			if($rtvFileObj->sumOracleData($_POST['requestId'])){
				$rtvFileObj->countOracleInvData($_POST['requestId']);
			}
		}
	exit();
	
	case "remvLoadedInv":
		$rtvFileObj->remvLoadedInv($_POST['requestId']);
	exit();
	break;	
	
	case "updFileName":
		$rtvFileObj->updFileName($_POST['requestId']);
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $rtvFileObj->viewFileName($_POST['requestId']);
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
					...
					<table border="0">
						<th colspan="3">
							<h3 align="center"><font style="font-family:Lucida Handwriting"> 901 - RTV (Textfiles) With Request Id </font></h3>
						</th>
						<tr>
