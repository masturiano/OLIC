...
class rtvFileObj extends commonObj {
	...
	function sumOracleData($reqId) {
		$sql="
		select sum(INVOICE_AMOUNT) AS INVOICE_AMOUNT from  tbl_ap_invoices_all_rtv_amount
		";
	...
	function countOracleInvData($reqId) {
		...
		(select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar) as uniqInv from tbl_ap_invoices_all_rtv_amount group by cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar)) as tbl_ap_invoices_all_rtv_amount
		";
