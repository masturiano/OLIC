...
switch($_POST['action']){
	...
	case "mmsData":
		$stsObj->mmsData($_POST['orgId']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$stsObj->mmsDataRemvSpace($_POST['orgId']);
	exit();
	break;	
    
    case "remvLoadedInvFirst":
        $stsObj->remvLoadedInvFirst($_POST['orgId']);
    exit();
    break; 
	
	case "oracleData":
		$stsObj->oracleData($_POST['orgId'],$_POST['clearOra']);
	exit();
	
	case "remvLoadedInv":
		$stsObj->remvLoadedInv($_POST['orgId']);
	exit();
	break;	
	
	case "updFileNameDuplicate":
		$stsObj->updFileNameDuplicate($_POST['orgId']);
	exit();
	break;	
    
    case "updFileNameCancelled":
        $stsObj->updFileNameCancelled($_POST['orgId']);
    exit();
    break;    
    
    case "updFileNameLoad":
        $stsObj->updFileNameLoad($_POST['orgId']);
    exit();
    break;    
	
	case "createTxtFile":
	
		$arrFileName = $stsObj->viewFileName($_POST['orgId']);
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $stsObj->viewSts($fileName,$_POST['orgId']);
        ...
        <script type="text/javascript">
			...
			function process()
			{
				...
				({
					url: 'sts.php',
					type: 'POST',
					data: 'action=mmsData&orgId='+orgId+'&clearOra='+clearOraInv,
					beforeSend: function()
					{
            ...
            function process8(orgId,clearOraInv)
            {
                ...
                ({
                    ...
                    success: function(data1){
                        $().toastmessage('showToast', {
                            text: 'Updating filename load success!',
                            sticky: false,
                            position: 'middle-center',
                            type: 'success',
                            closeText: '',
            ...
            <div id="activity_pane" align="center">
                ...
                <table border="0">
                    ...
                    <tr>
                        ...
                        <td><? $stsObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox"'); ?></td>
                    </tr>
                    <tr>
