...
class oracleDataObj extends commonObj {
	...
	function viewOracleDataInv() {
		...
		$sql="
        select Invoice as Col001,Vendor as Col002 from SID_050910 where Invoice <> '' group by Invoice,Vendor
		";
	...
	function oracleData() {
		...
		$truncTable = "truncate table tbl_ap_invoices_all_oracle_data";
		
		if($this->execQry($truncTable)){
		
			foreach($arrStsInv as $valConInv){
				$sql="
				insert into tbl_ap_invoices_all_oracle_data (INVOICE_NUM,SEGMENT1,INVOICE_AMOUNT)
