...
$conFileObj = new conFileObj();

switch($_POST['action']){
	
	case "concessData":
    
        $checkReqId = $conFileObj->checkReqId($_POST['requestId']);
        
        if($checkReqId['request_id'] == '1'){
            echo "1";
        }else{
            
            $conFileObj->insertReqId($_POST['requestId']);
		
		    $directory="C:/wamp/www/OLIC/con_data/";
			$arch_directory="C:/wamp/www/OLIC/con_archive/";
			...
			if ($checkEmpty == "Empty"){
						...
						if($file_ext == "601"){
							...
							if (($handle = fopen($csv_file, "r")) !== FALSE) {
								...
								while (($data = fgetcsv($handle, 10000000, "|")) !== FALSE) {
									...
									$conFileObj->concessData($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col15,$col16,$col17,$col18,$col19,$col20,$col21,$col22,$col23,$col24,$col25,$col26,$col27,$col28,$col29,$col30,$col31,$col32,$col33,$col34,$col35,$col36,$col37,$col38,$col39,$col40);
								}	
								if($loaded = $conFileObj->displayLoadedRec()){
									echo "
				...
				$dir = 'C:/wamp/www/OLIC/con_data/';
				foreach(glob($dir.'*.601*') as $v){
					unlink($v);
				}
				foreach(glob($dir.'*.601*') as $v){
					unlink($v);
	...
	case "mmsDataRemvSpace":
		$conFileObj->mmsDataRemvSpace();
	exit();
	break;	
	
	case "oracleData":
		$conFileObj->oracleData();
	exit();
	
	case "remvLoadedInv":
		$conFileObj->remvLoadedInv();
	exit();
	break;	
	
	case "updFileName":
		$conFileObj->updFileName();
	exit();
	break;	
	
	case "createTxtFile":
	
		$arrFileName = $conFileObj->viewFileName();
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $conFileObj->viewCon($fileName);
				...
				({
					...
					success: function(data1){
						if(data1 == 0){
							...
							$().toastmessage('showToast', {
								text: 'Copy 601 (Concess) data success!',
								sticky: false,
								position: 'middle-center',
								type: 'success',
								closeText: '',
        ...
        <body>
            ...
            <center>
            	<div id="activity_pane" style="height: 100vh;">
                    ...
                    <table border="0">
                    	<th colspan="3">
                        	<h3 align="center"><font style="font-family:Lucida Handwriting"> 601 - Concess (Textfiles) w/ Request Id</font></h3>
                        </th>
                        <tr>
