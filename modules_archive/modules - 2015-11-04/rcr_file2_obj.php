...
class rcrFileObj extends commonObj {
    ...
    function sumMmsData($reqId) {
        ...
        (select sum(cast(Col013 as float)) as uniqAmt,cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar) as uniqInv from tbl_rcr_invoice group by cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)) as tbl_rcr_invoice
        ";
    ...
    function countMmsInvData($reqId) {
        ...
        (select cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar) as uniqInv from tbl_rcr_invoice group by cast(Col001 as nvarchar)+'-'+cast(Col004 as nvarchar)) as tbl_rcr_invoice
        ";
	...
	function viewRcrInv($reqId) {
		
		$sql="
        select Col001,Col004,sum(cast(Col013 as float)) as Col013 from tbl_rcr_invoice group by Col001,Col004
		";
		...
		if($this->execQry($truncTable)){
			...
			foreach($arrRcrInv as $valRcrInv){
                ...
                select ORAPROD.INVOICE_NUM,ORAPROD.SEGMENT1,ORAPROD.INVOICE_AMOUNT from openquery(ORAPROD,
                    ...
                    and ap_invoices_all.INVOICE_AMOUNT = ''{$valRcrInv[Col013]}''
                    '
                    ) ORAPROD
                ";
    ...
    function sumOracleData($reqId) {
        $sql="
        select sum(INVOICE_AMOUNT) AS INVOICE_AMOUNT from  tbl_ap_invoices_all_rcr_amount
        ";
    ...
    function countOracleInvData($reqId) {
        ...
        (select cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar) as uniqInv from tbl_ap_invoices_all_rcr_amount group by cast(INVOICE_NUM as nvarchar)+'-'+cast(SEGMENT1 as nvarchar)) as tbl_ap_invoices_all_rcr_amount
        ";
