...
include("sid_date_range_ora_obj.php");

$olicDateRangeObj = new olicDateRangeObj();

switch($_POST['action']){
	
	case "mmsData":
		$olicDateRangeObj->mmsData($_POST['getTxtDateFrom'],$_POST['getTxtDateTo'],$_POST['orgId']);
	exit();
	break;	
	
	case "mmsDataRemvSpace":
		$olicDateRangeObj->mmsDataRemvSpace($_POST['orgId']);
	exit();
	break;	
    
    case "remvLoadedInvFirst":
        $olicDateRangeObj->remvLoadedInvFirst($_POST['orgId']);
    exit();
    break; 
	
	case "oracleData":
		$olicDateRangeObj->oracleData($_POST['orgId'],$_POST['clearOra']);
	exit();
	break;	
	
	case "remvLoadedInv":
		$olicDateRangeObj->remvLoadedInv($_POST['orgId']);
	exit();
	break;	
	
	case "updFileNameDuplicate":
		$olicDateRangeObj->updFileNameDuplicate($_POST['orgId']);
	exit();
	break;	
    
    case "updFileNameCancelled":
        $olicDateRangeObj->updFileNameCancelled($_POST['orgId']);
    exit();
    break;    
    
    case "updFileNameLoad":
        $olicDateRangeObj->updFileNameLoad($_POST['orgId']);
    exit();
    break;    
	
	case "createTxtFile":
	
		$arrFileName = $olicDateRangeObj->viewFileName($_POST['orgId']);
		
		foreach($arrFileName as $valFileName){
			
			$fileName = $valFileName['Col040'];
			
			$arrFileCont = $olicDateRangeObj->viewOlic($_POST['orgId'],$fileName);
            ...
            <div id="activity_pane" align="center">
                ...
                <table border="0">
                    <tr>
                        <td colspan="3" align="center">  
                            <font style="font-family:Lucida Handwriting; font-size: 15px;"> 
                                <b>001 - SID File Date Range Ora</b>
                            </font>   
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><? $olicDateRangeObj->DropDownMenu($arrOrgId,'cmbOrgId','','class="selectBox"'); ?></td>
                    </tr>
                    <tr>
